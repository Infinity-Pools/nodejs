...
sudo DEBIAN_FRONTEND=noninteractive apt-get -y install git python-virtualenv python3-virtualenv curl ntp build-essential screen cmake pkg-config libboost-all-dev libevent-dev libunbound-dev libminiupnpc-dev libldns-dev libexpat1-dev libgtest-dev mysql-server lmdb-utils doxygen graphviz
cd ~
...
cd solace
sudo make release-static -j$(nproc)
...
sudo -u solacedaemon /usr/local/src/solace/build/release/bin/solace-blockchain-import --input-file $BLOCKCHAIN_DOWNLOAD_DIR/blockchain.raw --verify 1 --data-dir /home/solacedaemon/.solace
